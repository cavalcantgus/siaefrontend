      ...
      <v-row class="pa-3">
        <v-card class="custom-scroll elevation-0" width="85%" height="150px" style="overflow-y: auto; max-height: 250px; border: 1px solid #ccc">
          <v-card-text>
            ...
            <v-row>
              <v-col v-for="(membro, index) in currentItem.membros" :key="index" cols="12" md="6" lg="6" class="d-flex align-center">
                <v-text-field style="margin-left: 50px" v-model="currentItem.membros[index]" variant="outlined" density="compact" clearable :label="`Membro ${index + 1} *`"></v-text-field>
                <v-btn :disabled="this.currentItem.membros.length > 1 ? false : true" :color="this.currentItem.membros.length > 1 ? 'error' : 'gray'"  size="30px" icon class="ml-2 mb-6" @click="removeItem(index)" style="position: absolute">
...
export default {
  ...
  methods: {
    addItem() {
      ...
      this.currentItem.membros.push(null);
    },
    removeItem(index) {
      this.currentItem.membros.splice(index, 1);
    },
